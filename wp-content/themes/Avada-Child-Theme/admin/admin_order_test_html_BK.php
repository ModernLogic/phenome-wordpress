   ...
   // print_r($meta_array); // For debugging
		...
		$dd=array();
			foreach ($formatted_meta as $key => $details){
				...
				$dd[]=array($details->key,$details->value);
...
function remove_ccbproajaxactions_hooks() {
    ...
    remove_filter('woocommerce_order_item_meta_end', array('cBuilder\Classes\CCBWooCheckout', 'calc_order_item_meta'), 99);
    
    // Remove the 'woocommerce_after_order_itemmeta' action added by CCBWooCheckout::calc_order_item_meta
    remove_action('woocommerce_after_order_itemmeta', array('cBuilder\Classes\CCBWooCheckout', 'calc_order_item_meta'), 99);
}
...
//print_r($groupedData);
				 ...
				 <tr><td colspan="6"><div class="metacollapsible__wrap">
					...
					if($value2!=''){
					  ...
					  if($snakeitem!=''){
											...
											echo '<tr><td>'.$snakeitem.'</td><td>'.($currenttesresult!=''?'Complete':'Pending').'</td><td style="text-align: right;">';?>
    											<select data-parent="<?php echo $i;?>" data-index="<?php echo $key;?>" data-itemid="<?php echo $item_id;?>" class="add_result_meta" data-type="_sub_test_result" name="sub_test_results">
					 ...
					 if($value!=''){
										 $currenttesresult = wc_get_order_item_meta( $item_id, '_main_test_result_'.$i.'_0' );
											echo '<tr><td>'.$value.'</td><td>'.($currenttesresult!=''?'Complete':'Pending').'</td><td style="text-align: right;">';?>
												<select  data-parent="<?php echo $i;?>" data-index="0"  data-itemid="<?php echo $item_id;?>" data-type="_main_test_result" class="add_result_meta" name="main_test_results">
												<option>Select</option>
													<?php
													// Loop through the options array to generate select options
													foreach ($options as $option) {
